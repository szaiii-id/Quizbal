...
class QuizApp:
    ...
    def load_sounds(self):
        try:
            ...
            print(f"Peringatan: Gagal memuat efek suara. {e}")
    ...
    def play_bgm(self, music_file):
        try:
            ...
            print(f"Peringatan: Gagal memuat BGM '{music_file}'. {e}")
    ...
    def create_game_selection(self):
        ...
        tk.Button(frame, text="Kembali ke Menu", font=("Arial", 12), 
                  ...
                  command=self.back_to_menu).pack(pady=30) 
    ...
    def start_quiz(self, game_type_key):
        ...
        if game_type_key == "gambar":
            self.play_bgm(PATH_MUSIK_GAMBAR)
        elif game_type_key == "umum":
            self.play_bgm(PATH_MUSIK_UMUM)
        elif game_type_key == "hewan":
            self.play_bgm(PATH_MUSIK_HEWAN)
        elif game_type_key == "matematika":
            self.play_bgm(PATH_MUSIK_MATEMATIKA)
    ...
    def check_answer(self, selected_index):
        ...
        if selected_option == correct_answer:
            self.score += 1
            if self.suara_benar:
                self.suara_benar.play()
            self.create_feedback_popup("Hore! Benar! 👍", WARNA_TOMBOL_BENAR)
            selected_btn.config(bg=WARNA_TOMBOL_BENAR, fg=WARNA_TEKS_BIASA)
        else:
            if self.suara_salah:
                self.suara_salah.play()
            self.create_feedback_popup("Oops, Salah!", WARNA_TOMBOL_SALAH)
    ...
    def create_feedback_popup(self, message, color):
        ...
        text_color = WARNA_TEKS_BIASA if color == WARNA_TOMBOL_BENAR else "white"
        
        label = tk.Label(popup, text=message, font=label_font, 
                         bg=color, fg=text_color)
        label.pack(pady=30, expand=True, fill="both")
